...
const loadPlaylist = async (endpoint, elementId) => {
  ...
  for (let { name, description, images, id } of items) {
    ...
    playlistItem.innerHTML = `<img
        src="${imageUrl}" 
        alt="${name}" class = "rounded mb-2 object-contain shadow"
      />
...
const loadPlaylistTracks = ({ tracks }) => {
  ...
  for (let trackItem of tracks.items.filter((item) => item.track.preview_url)) {
    ...
    //   playButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
const findCurrentTrack = () => {
  ...
  if (trackId) {
    ...
    return { currentTrackIndex, track: loadedTracks };
  }
